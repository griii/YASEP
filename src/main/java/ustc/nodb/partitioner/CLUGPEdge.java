...
public class CLUGPEdge implements PartitionStrategy
{
    ...
    public void performStep(PartitionResBuffer partitionResBuffer) {
        Edge edge;
        List<String> curBuffer = new ArrayList<>();
        while ((edge = this.batchGraph.readStep()) != null) {
            ...
            curBuffer.add(src + " " + dest + " " + n);
            if (curBuffer.size() >= 1024){
                partitionResBuffer.addList(curBuffer);
                curBuffer.clear();
            }
//            System.out.println("src:" + src + "dst: " + dest + "partition:" + n);
